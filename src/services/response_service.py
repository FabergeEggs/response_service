...
from src.services.kafka_producer_protocol import KafkaProducer  
...
class ResponseService:
    ...
    async def get_task_responses(self, task_id: UUID) -> List[Response]:
        ...
        responses = await self._repo.get_task_responses(task_id)
        return await self._attach_user_names(responses)

    async def _attach_user_names(self, responses: List[Response]) -> List[Response]:
        if not responses or self._denorm_repo is None:
            return responses
        user_ids = list({response.user_id for response in responses})
        names = await self._denorm_repo.get_user_names(user_ids)
        return [
            response.model_copy(
                update={"user_name": names.get(response.user_id, "") or ""}
            )
    ...
    async def get_response(self, response_id: UUID) -> Optional[Response]:
        ...
        if response is None:
            return None
        enriched = await self._attach_user_names([response])
        return enriched[0]

    async def add_response(self, response: Response) -> Response:
        if self._denorm_repo is not None:
            await self._denorm_repo.upsert_user(response.user_id)
            await self._denorm_repo.upsert_task(response.task_id)
